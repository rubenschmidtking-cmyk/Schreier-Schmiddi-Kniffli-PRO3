...
export default function OnlineMatch({ code, role, settings, onExit }: Props) {
  ...
  useEffect(() => {
    ...
    if (!realtimeConfigured || !client) return
    const channel = client.channel(`kniffli:${code.toUpperCase()}`, { config: { presence: { key: clientId }, broadcast: { self: false } } })
    ...
    return () => { document.removeEventListener('visibilitychange', visibility); void client.removeChannel(channel); channelRef.current = null }
