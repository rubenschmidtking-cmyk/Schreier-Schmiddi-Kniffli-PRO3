...
export function playCpuRound(player: PlayerState, difficulty: Difficulty): CpuRoundResult {
  ...
  while (rolls < 3 && !chosen) {
    dice = dice.map((value, i) => held.has(i) ? value : Math.floor(Math.random() * 6) + 1)
    rolls += 1
    const scored = [...open].map(category => ({ category, points: scoreCategory(category, dice) }))
    scored.sort((a,b) => weighted(b.category,b.points,difficulty) - weighted(a.category,a.points,difficulty))
    const best = scored[0]
    const bank = best.points >= 40 || (rolls === 2 && best.points >= (difficulty === 'easy' ? 18 : 25)) || rolls === 3
    if (bank) chosen = best.category
    else held = chooseHolds(dice, open, difficulty)
  }

  chosen ??= [...open][0]
  const points = scoreCategory(chosen, dice)
  return {
    category: chosen,
    points,
    finalDice: dice,
