...
export function rollDice(player: PlayerState): PlayerState {
  ...
  return {
    ...
    dice: player.dice.map(die => die.held ? die : { ...die, value: Math.floor(Math.random() * 6) + 1 }),
  }
}
...
export function submitCategory(player: PlayerState, category: ScoreCategory): { player: PlayerState; points: number } {
  if (player.rollsUsed === 0 || player.submitted || player.scoreCard[category] !== null) return { player, points: 0 }
  const points = scoreCategory(category, player.dice.map(d => d.value))
  return {
    points,
